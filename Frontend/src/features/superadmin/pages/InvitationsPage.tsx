...
export function InvitationsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    expiry.setDate(expiry.getDate() + form.expiry_days);
    ...
    setShowForm(false);
    setForm({ org_name: "", admin_name: "", admin_email: "", subscription_plan: "starter", expiry_days: 7 });
  };
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border p-6" style={{ borderColor: "#E3E9F6" }}>
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: "#374151" }}>Subscription Plan</label>
              <select
                className="w-full border rounded-xl px-3 py-2 text-sm outline-none focus:ring-2"
                style={{ borderColor: "#E3E9F6", color: "#111827" }}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: "#F8FAFF", borderBottom: "1px solid #E9EEF8" }}>
              ...
              <th className="text-left px-5 py-3 text-xs font-semibold" style={{ color: "#6B7280" }}>Admin Email</th>
              <th className="text-left px-5 py-3 text-xs font-semibold" style={{ color: "#6B7280" }}>Plan</th>
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-sm" style={{ color: "#9CA3AF" }}>Loading…</td>
              </tr>
            ) : isError ? (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-sm" style={{ color: "#EF4444" }}>Failed to load</td>
              </tr>
            ) : invitations.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-sm" style={{ color: "#9CA3AF" }}>No invitations yet</td>
              </tr>
            ) : (
              invitations.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="border-t hover:bg-gray-50" style={{ borderColor: "#F3F4F6" }}>
                    ...
                    <td className="px-5 py-3.5" style={{ color: "#374151" }}>{inv.admin_email}</td>
                    <td className="px-5 py-3.5 capitalize" style={{ color: "#374151" }}>{inv.subscription_plan}</td>
