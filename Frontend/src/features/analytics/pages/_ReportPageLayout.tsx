...
import {
  Download, FileText, Trash2, BarChart3,
...
import type { GeneratedReport, ReportType, ReportFormat, ReportStats } from "@/features/analytics/api/reportsApi";
...
function StatsRow({ type, data }: { type: ReportType; data: ReportStats }) {
  if (type === "kpi") {
    const d = data.kpi;
    return (
  ...
  if (type === "workforce") {
    ...
    return (
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard label="TRIR"                 value={d.incident_rate}        color="#F97316" sub="per 200k hrs" />
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="!p-0 overflow-hidden">
        ...
        {histLoading ? (
          <div className="p-5 text-sm text-center py-6" style={{ color: "#9CA3AF" }}>Loading…</div>
        ) : reports.length === 0 ? (
...
export function ReportPageLayout({ reportType, title, description, icon: Icon }: Props) {
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "#111827" }}>{title}</h1>
          <p className="text-sm mt-0.5" style={{ color: "#6B7280" }}>{description}</p>
        </div>
      </div>

      {!statsLoading && stats && <StatsRow type={reportType} data={stats} />}

      <ReportPanel reportType={reportType} title={title} />
    </div>
  );
}
