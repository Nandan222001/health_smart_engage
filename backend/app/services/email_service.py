    ...
    *,
    ...
    try:
        ...
        if response.status_code in (200, 202):
            logger.info("[email] Invitation sent to %s — status %s", admin_email, response.status_code)
            return True
        else:
            logger.error("[email] SendGrid rejected email to %s — %s: %s", admin_email, response.status_code, response.text)
            return False
    except Exception as exc:
        print(f"[SendGrid] EXCEPTION: {exc}", flush=True)
        logger.error("[email] Failed to send invitation to %s: %s", admin_email, exc)
        return False
